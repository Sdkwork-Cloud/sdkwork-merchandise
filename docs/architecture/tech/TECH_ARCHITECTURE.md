...
`sdkwork-merchandise` is a **T1 capability repository** in the commerce domain. It owns domain services, SQL repositories, HTTP route builders, and a standalone gateway with IAM middleware. The `sdkwork-commerce (deleted)` monolith has been dissolved; each T1 capability repository is self-contained.
...
| Layer | Owner | Notes |
| --- | --- | --- |
| Domain commands/queries | `sdkwork-merchandise-service` | Business validation and ports |
| SQL repositories | `sdkwork-commerce (deleted)-merchandise-repository-sqlx` | Tenant-scoped persistence |
...
- `crates/sdkwork-commerce (deleted)-merchandise-repository-sqlx/`
