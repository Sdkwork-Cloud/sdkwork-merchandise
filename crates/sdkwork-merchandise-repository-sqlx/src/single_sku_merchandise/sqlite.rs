...
pub(super) async fn list(
    ...
    let rows = sqlx::query(
        ...
        WHERE sku.tenant_id = ?1
          AND (?2 IS NULL OR sku.organization_id = ?2)
          AND LOWER(COALESCE(sku.fulfillment_type, '')) = LOWER(?3)
          AND (?4 IS NULL OR LOWER(COALESCE(sku.status, '')) = LOWER(?4))
          AND (
                ?5 IS NULL
    ...
    let mut items = rows
        .iter()
        .map(crate::sqlite_catalog::map_sku_row)
        .collect::<Vec<_>>();
...
pub(super) async fn create(
    ...
    sqlx::query(
        ...
        VALUES (?1, ?2, ?3, ?4, ?5, ?5, ?6, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
    ...
    sqlx::query(
        ...
        VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?6, ?7, ?8, ?9, ?10, 'untracked', ?11, ?12,
...
pub(super) async fn update(
    ...
    .bind(i64::from(command.original_price_amount.is_some()))
    .bind(
        command
            .original_price_amount
            .as_ref()
...
async fn load_sku(
    ...
    Ok(row.as_ref().map(crate::sqlite_catalog::map_sku_row))
}
...
async fn load_sku_by_no(
    ...
    let row = sqlx::query(
        ...
        SELECT id, tenant_id, organization_id, spu_id, sku_no,
               ...
               CAST(updated_at AS TEXT) AS updated_at
        FROM commerce_product_sku
        WHERE tenant_id = ?1 AND organization_id = ?2 AND sku_no = ?3
        LIMIT 1
        "#,
    )
    .bind(tenant_id)
...
async fn load_spu_by_no(
    ...
    let row = sqlx::query_as::<_, (String, String, String, String)>(
        r#"
        SELECT CAST(id AS TEXT), spu_no,
               COALESCE(NULLIF(title, ''), name, CAST(id AS TEXT)) AS title,
               COALESCE(status, 'draft') AS status
        FROM commerce_product_spu
        WHERE tenant_id = ?1 AND organization_id = ?2 AND spu_no = ?3
        LIMIT 1
        "#,
    )
    .bind(tenant_id)
...
async fn ensure_single_sku_spu(
    ...
    let count = sqlx::query_scalar::<_, i64>(
        ...
        SELECT COUNT(*)
        FROM commerce_product_sku
        WHERE tenant_id = ?1 AND organization_id = ?2 AND spu_id = ?3
        "#,
    )
    .bind(tenant_id)
    .bind(organization_id)
